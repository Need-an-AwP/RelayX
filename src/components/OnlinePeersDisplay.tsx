...
import { Laptop, Server, Smartphone, Globe, Wifi } from 'lucide-react'; // 图标库
                ...
                {allPeerList.length === 0 ? (
                    ...
                    <ScrollArea className="h-full w-full">
                        {allPeerList.map((peer) => {
                            const user = users.get(peer.TailscaleIPs[0]);

                            return (
                                <div key={peer.ID || peer.HostName} className="border rounded-md space-y-2 p-2 mb-2 mx-2 text-xs hover:bg-muted/50 ">

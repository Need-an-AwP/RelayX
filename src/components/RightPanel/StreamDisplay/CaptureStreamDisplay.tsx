...
import { useDesktopCapture, usePeerStateStore } from "@/stores"
import {
...
export default function CaptureStreamDisplay() {
    const { stream, setStream } = useDesktopCapture((state) => state)
    const { updateSelfState } = usePeerStateStore((state) => state)
